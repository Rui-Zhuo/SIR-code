...
c ist=1 (i); =2 (q); =3 (u); =4 (v)
        ...
        subroutine blends2(atmos,stok,rt,rp,rh,rv,rg,rf,rm,mnodos)
...
c               AQUI! FALTA DECLARAR ESTAS VARIABLES
...
c	         print*,'11 llamo a leelineasii con nxx=',nxx
                 call leelineasii(nxx,atom,istage,wlengt,zeff,energy,
     &                         loggf,mult,design,tam,alfa,sigma)
c                 print*,'1 blends2.f iln nble(iln) ible nxx',iln,nble(iln),ible,nxx
              end if   
              gf=1.e1**(loggf)
...
c a sumatorio y normalizmos por el continuo
     ...
     &             call nodos_mult(grp,ntau,tau,pe,mnodos(2))  !Warning: interpolating the relative perturbation       
              end if
...
c              if(isv .eq. 1)print*,'blends2 ikk4',ikk4,stok(ikk4)
              end if
            end do
...
c __________________________________________________________________________
c rutina nodos_mult evaluates the equivalent RF at nodes 
c CONSIDERING that the multiplicative perturbations will be interpolated 
C 

        subroutine nodos_mult(gr,n,tau,yy,no)
        ...
        if(no.eq.1)then   !si perturbacion constante
           ...
           yymedio=0.
           do i=1,n
               sumagr=sumagr+gr(i)*yy(i)  !pert. multiplicativa constante
c               sumagr=sumagr+gr(i)        !pert. aditiva constante
c               yymedio=yymedio+yy(i)      !necesario si pert. aditiva constante
           end do
           gr(1)=sumagr                  !pert. multiplicativa constante
c           gr(1)=sumagr*yymedio/float(n)  !necesario si pert. aditiva constante: escalamos al velor medio del parametro
           ...
           call splines22(x,y,no-2,n,tau,zz,f)  !f is the interpolatedvparameter
                      
           do i=1,no
              ggg=0.
              do j=1,n
                 ggg=ggg+gr(j)*f(j,i)*yy(j)
              end do
              g(i)=ggg
           end do
           do i=1,no
             gr(i)=g(i)
c             print*,'blend2 new i gr=',i,gr(i)
           end do
...
c __________________________________________________________________________
c rutina nodos calcula las funciones respuesta equivalentes en los nodos 
c version para la Pe :escala con la presion en el nodo mas profundo

        subroutine nodosp(gr,n,tau,yy,no)
